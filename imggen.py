...
def imggen():
    ...
    leaderimg = Image.new("RGBA", (400,18), (200,240,255,255))
    draw = ImageDraw.Draw(leaderimg)
    draw.text((0,0), "Rank  Coin  Marketcap  Price(USD)", (0,0,0), font=font)
    ...
    for idx,val in enumerate(cmcdata):
        if idx == 0:
            img = Image.new("RGBA", (400,18), (200,255,200,255))
        else:
            img = Image.new("RGBA", (400,18), (255,255,255,255))
    ...
    x_offset = 0
    tickerimg = Image.new("RGBA", (sum(widthlist)+10*(spacing+logosize)+bufferspace,18), (255,255,255,255))
    ...
    for idx,val in enumerate(toptenlist):
        ...
        tickerimg.paste(coinimg, (x_offset+spacing, 1))
        tickerimg.paste(textimg, (x_offset+spacing+logosize, 0))
        x_offset += widthlist[idx+1] + 76
